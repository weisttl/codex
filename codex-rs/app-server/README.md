...
- `thread/contextInspect` — experimental; inspect a loaded thread's current model-context history and latest model request observations without starting a turn or mutating the thread. The response contains metadata-only projections, fingerprints, serialized sizes, and transport metadata. Optional `maxItems` limits item metadata records and is clamped to a server hard limit; complete content, arguments, outputs, schemas, and IDs are not returned.
...
Paginated threads require `excludeTurns: true`. Their response leaves `thread.turns` empty and includes `turnsBackwardsCursor` and `itemsBackwardsCursor` for the durable history visible at the resume boundary. Pass each cursor directly to its matching list API with `sortDirection: "desc"`; the first page includes the cursor's head row, while newer records arrive through live notifications. Either cursor is `null` when there is no durable row yet. Full-history resume and `initialTurnsPage` remain unavailable for paginated threads.
...
Use `thread/contextInspect` with `capabilities.experimentalApi = true` to inspect a thread that is currently loaded. This is a read-only, metadata-only view: `raw` describes recorded history, while `normalized` describes that same history after production prompt normalization. `request.latestAttempt` describes the latest prepared, sent, or failed request attempt; `request.lastActual` is the latest request successfully sent at the transport boundary. For HTTP this means a response stream was returned; for WebSocket it means the request frame was written successfully, not that the provider accepted it. Each request contains separate pre-adaptation `normalizedInput` and complete logical `providerInput` projections. For WebSocket requests, `transportUsesDelta` and `transportInputItems` describe the actual delta transport while the request fingerprint and `providerInput` continue to describe the complete logical request. Connection-only `generate=false` prewarm requests are excluded. Pending input and future turn injections are not included, and tool schemas, base instructions, output schemas, content, arguments, outputs, and IDs are represented only by metadata rather than returned verbatim.
